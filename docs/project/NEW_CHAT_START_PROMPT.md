# NEW CHAT START PROMPT — Four Elements Remake



Ты работаешь с проектом **Four Elements Remake** — браузерная изометрическая RTS.
...
Mirror только для чтения. Патчить его напрямую нельзя.



## 2. Что читать в начале
...
Если mirror stale — запросить свежий:
...
<!-- FE_NEW_CHAT_FRESHNESS_RULE_START -->
## 2.1. Freshness rule

Перед продолжением проекта не опираться на память старого чата. Сначала сверить mirror/context. Если mirror stale/неполный или конфликтует с архивом — повторить чтение/поиск актуального контекста. Если уверенности нет — запросить свежий `_exports\GPT_WORK_SEND_THIS_CONTEXT.zip`.

Запрещено уверенно называть рабочую папку, patch status или следующий шаг, если это не подтверждено свежими файлами.

<!-- FE_NEW_CHAT_FRESHNESS_RULE_END -->

<!-- FE_DOCS_CADENCE_RULE_START -->
## 2.2. Documentation cadence rule

После каждых двух принятых успешных патчей нужно сначала обновить документацию, а уже потом начинать новую feature/code задачу.

Принятый успешный патч = применён через runner, прошёл проверки, пользователь подтвердил результат, после этого выполнен sync mirror.

Если после последнего docs patch уже есть два успешных patch-result подряд, GPT должен сам предложить docs patch и не ждать напоминания пользователя.
...
- игра стартует после выбора фракции;
...
- enemy units/HQ имеют `owner='enemy'`, визуально `faction='purple'`;
- enemy units не раскрывают fog игроку;
...
- player light_tank может атаковать enemy HQ;
- клик по земле после attack-order отменяет старую цель;
- ПКМ после attack-order тоже отменяет старую цель;
- повторный приказ атаки по enemy HQ снова работает;
- victory/defeat overlay работает;
- `Esc` после victory/defeat ведёт в главное меню;
- debug-toast `Enemy HQ spawned` убран.

Важно: enemy bot — MVP phase controller, не полноценный AI. Enemy economy/production postponed to 09A+.

## 4. Codex policy



Codex беречь. Не использовать по умолчанию.



Codex разрешён только с явным маркером:
...
Мелкие docs/config/UI-state/local patches делает GPT через `patch.py`.



Codex нужен позже для:
...
## 5. Следующий крупный блок



Не лезть сразу в код бота без отдельного аудита.



Следующий крупный Codex-кандидат:
...
- убрать тупой постоянный rush;

- сделать состояния `idle / defend / attack / retreat`;

- добавить защиту enemy HQ;

- выбирать цель owner-based;

- не добавлять пока экономику/производство.



## 6. Ближайшие безопасные локальные задачи



Если надо продолжить без Codex:
...
Не трогать combat/movement/pathfinding/bot AI без свежего контекста и явного решения.

<!-- FE_PATCH_08A0_ASHEN_CROWN_AI_AUDIT_DOCS_START -->
## Ashen Crown AI audit context

The user uploaded and reviewed `ashen-crown-main.zip` as a reference RTS project. A project-specific audit was created and stored at:

```text
docs/project/ashen_crown_ai_audit_for_four_elements.md
```

Key conclusion: do not copy Ashen Crown code directly. Use it as an architecture reference for bot behavior.

Useful bot ideas for Four Elements:

- `defend / prepare_attack / attack / regroup` phases;
- `armyScore` instead of fixed unit-count attack triggers;
- defend enemy HQ when player units pressure it;
- regroup after failed attack or over-chase;
- decision tick every ~1-1.5 seconds, not every frame;
- AI chooses intent, existing movement/combat helpers execute it.

Completed read-only audit:

```text
PATCH-08C-BOT-ROUTE-STABILITY-AUDIT
```

Synced audit report:

```text
_inbox/session_summary_20260508_PATCH-08C-BOT-ROUTE-STABILITY-AUDIT.txt
...
Enemy economy, harvesting, production, and faction personalities are postponed to 09A+.
...
## Current checkpoint for new chats

Current accepted docs checkpoint:

```text
PATCH-09C4-DOCS-ENEMY-SEPARATOR-CHECKPOINT
```

Accepted gameplay patches since previous docs sync:
- `PATCH-09C2-ENEMY-BUILDER-BUILD-SEPARATOR-MVP`: enemy builder builds enemy-owned separator instead of spawning it from air.
- `PATCH-09C3-ENEMY-SEPARATOR-PROCESSING-MVP`: completed enemy separator processes hidden enemy resources using `15 raw minerals -> 10 energy + 1 purple element`.

Next route:
1. `PATCH-09C5-ENEMY-START-ENERGY-160` — align enemy hidden start energy with player baseline as a temporary skirmish baseline.
2. `PATCH-09D-ENEMY-FACTORY-BUILD-MVP` — enemy builder builds `units_factory`.
3. `PATCH-09E-ENEMY-FACTORY-LIGHT-TANK-PRODUCTION-MVP` — enemy factory produces `light_tank` through faction elements.

Do not spawn enemy combat units directly from HQ.

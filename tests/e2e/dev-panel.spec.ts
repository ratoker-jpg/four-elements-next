...
test.describe('DEV-SANDBOX-TOOLS-01 dev panel tools', () => {
  ...
  test('dev panel opens with backtick key', async ({ page }) => {
    await navigateToGameScreen(page);

    // Dev panel should be hidden initially
  ...
  test('+ Obstacle at camera center increases obstacle count if free', async ({ page }) => {
    ...
    const freeTile = await page.evaluate(() => {
      ...
      // Use builder position as a starting point — tiles near idle builders are usually free
      // Or just find a tile that is not occupied by checking the __harvesterState nodes + __constructionState
      // Simplest: try tile at (1, map_height-2) — bottom-left corner, usually sand with nothing
      const mapW = 48; // standard map
      const mapH = 48;
      // Return a tile in the bottom-left corner
    ...
    // Add obstacle — if the tile is still occupied, this is a no-op
    // Try multiple tiles in the corner area
    let added = false;
